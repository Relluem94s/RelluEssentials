...
public class Teleport implements CommandConstruct {
    ...
    private void addTeleportEntry(Player p, Player t) {
        ...
        t.sendMessage(languageHelper.getWithPrefix(MessageKey.COMMAND_TP_REQUEST_TARGET, p.getCustomName()));
    ...
    public boolean onCommand(@NonNull CommandSender sender, @NotNull Command command, @NonNull String label, String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (!Permission.isAuthorized(p, Groups.getGroup("mod").getId())) {
                ...
                p.sendMessage(languageHelper.getWithPrefix(MessageKey.COMMAND_TP_SEND_REQUEST, target.getCustomName()));
                return true;
            }
        ...
        if (args.length == 2) {
            ...
            if (target == null) {
                p.sendMessage(languageHelper.getWithPrefix(MessageKey.COMMAND_TARGET_NOT_A_PLAYER, args[1]));
                return true;
            }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        if(strings.length == 1){
            ...
            Bukkit.getOnlinePlayers().stream()
                    .filter(player -> !player.equals(commandSender))
                    .map(Player::getName)
                    .forEach(tabList::add);
        }
