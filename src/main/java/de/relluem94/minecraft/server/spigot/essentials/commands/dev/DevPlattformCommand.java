...
public class DevPlattformCommand implements SubCommand {
                    ...
                    for (int dz = -2; dz <= 2; dz++) {
                        ...
                        if (Math.abs(dx) == 2 || Math.abs(dz) == 2) {
                            ...
                            if (dx == 0 && dz == 0) {
                                ...
                                Location redstoneLocation = world.getBlockAt(bx, originY, bz).getLocation().clone().subtract(0, 1, 0);
                                redstone.addLocation(redstoneLocation, schedule);
                                undoList.add(new ModifyHistoryEntry(redstoneLocation, redstoneLocation.getBlock().getType(), redstoneLocation.getBlock().getBlockData()));
                            } else {
    ...
    public boolean matches(String @NonNull [] args) {
        return args.length == 1 && DevCommand.Commands.DEV_PLATTFORM.getName().equalsIgnoreCase(args[0]);
    }
}