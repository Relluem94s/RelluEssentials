...
public class NPCValidator {
    ...
    public ValidationResult validateProfileName(String profileName) {
        ...
        if (profileName.length() < MINECRAFT_USERNAME_MIN_LENGTH || profileName.length() > MINECRAFT_USERNAME_MAX_LENGTH) {
            return ValidationResult.failure(
                    "Profile name must be between " + MINECRAFT_USERNAME_MIN_LENGTH + " and " + MINECRAFT_USERNAME_MAX_LENGTH + " characters."
            );
        }
        if (!profileName.matches("[a-zA-Z0-9_]+")) {
            return ValidationResult.failure("Profile name may only contain letters, digits, and underscores.");
        }
    ...
    public ValidationResult validateCoordinates(double x, double y, double z) {
        if (x < MINECRAFT_MIN_COORDINATE || x > MINECRAFT_MAX_COORDINATE) {
            return ValidationResult.failure("X coordinate is out of bounds (" + MINECRAFT_MIN_COORDINATE + " to " + MINECRAFT_MAX_COORDINATE + ").");
        }
        if (y < MINECRAFT_MIN_Y || y > MINECRAFT_MAX_Y) {
            return ValidationResult.failure("Y coordinate is out of bounds (" + MINECRAFT_MIN_Y + " to " + MINECRAFT_MAX_Y + ").");
        }
        if (z < MINECRAFT_MIN_COORDINATE || z > MINECRAFT_MAX_COORDINATE) {
            return ValidationResult.failure("Z coordinate is out of bounds (" + MINECRAFT_MIN_COORDINATE + " to " + MINECRAFT_MAX_COORDINATE + ").");
        }
        return ValidationResult.success();
    }
