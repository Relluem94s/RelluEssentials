...
public class PermissionsGroup implements CommandConstruct {
    ...
    private static void setGroupForTarget(@NotNull CommandSender s, @NotNull GroupEntry g, @NotNull OfflinePlayer target) {
        s.sendMessage(languageHelper.getWithPrefix(MessageKey.COMMAND_SETGROUP, g.getPrefix() + g.getName(), target.getName()));
        if(target.isOnline() && Bukkit.getPlayer(target.getUniqueId()) != null){
            Objects.requireNonNull(Bukkit.getPlayer(target.getUniqueId()))
                    .sendMessage(languageHelper.getWithPrefix(MessageKey.COMMAND_SETGROUP, g.getPrefix() + g.getName(), target.getName()));
        }
