...
/**
 * Command implementation that sets the game mode of a player to {@link GameMode#SPECTATOR}.
 *
...
public class GameModeSpectator implements CommandConstruct {
  ...
  /**
   * Handles the execution of the spectator game mode command.
   *
   * <p>If one argument is provided, the game mode of the specified target player is set to
   * {@link GameMode#SPECTATOR}. If no argument is provided, the sender itself is used as
  ...
  private void gameMode(@NotNull Player p) {
    ...
    p.sendMessage(serviceContext.getTranslationService()
        .getWithPrefix(MessageKey.COMMAND_GAMEMODE, p.getCustomName(),
            serviceContext.getTranslationService().get(MessageKey.COMMAND_GAMEMODE_SPECTATOR)));
  }

  /**
   * Provides tab completion suggestions for the spectator game mode command.
   *
